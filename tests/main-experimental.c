...
struct list_experimental_data* new_list() {
    // init list
    struct list_experimental_data* ptr = _new();
    // setup stack
    ptr->data[0] = ptr->data;
...
void delete_list(struct list_experimental_data** ctx) {
    // gets pointer
    struct list_experimental_data* ptr = *ctx;
    // destroys list
    _delete(ptr);
    // cleans up
    *ctx = 0;
...
void array_print_head(struct list_experimental_data** const current) {
    // get current context's head
    const struct list_experimental_data* ptr = *current;
    // gets offset
...
void array_print(struct list_experimental_data** const current) {
    // get current context's head
    const struct list_experimental_data* ptr = *current;
    // sets the counter
    int i = 0;
...
void using_list(void (*list_using)(struct list_experimental_data** const)) {
    // initialize current context (stack)
    struct list_experimental_data* ctx = new_list();
...
// default list usage scenario
void using_list2(void (*list_using)(struct list_experimental_data** const)) {
    // initialize current context (stack)
    struct list_experimental_data* ctx = new_list();
...
// use list
void list_using(struct list_experimental_data** const current) {
...
typedef struct test_data {
    struct list_experimental_data* ctx;
} *TEST_DATA;

/* Initialize the data structure. Its allocation is handled by Rexo. */
RX_SET_UP(test_set_up) {
    TEST_DATA rx = (TEST_DATA)RX_DATA;
    struct list_experimental_data** ctx = &rx->ctx;
    // initialize list
    *ctx = new_list();
    return RX_SUCCESS;
}
...
RX_TEST_CASE(myTestSuite, test_empty_list_pop_equals_0, .fixture = test_fixture) {
    struct list_experimental_data* ctx = 0;

    // create list
    const struct list* list = &list_experimental_definition;
    const struct list_experimental_data* head = list->pop(&ctx);
...
RX_TEST_CASE(myTestSuite, test_empty_list_peek_equals_0, .fixture = test_fixture) {
    ...
    // create list
    const struct list* list = &list_experimental_definition;
    const struct list_experimental_data* head = list->peek(&ctx);
