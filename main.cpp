...
struct qtype; typedef struct q_type* q_type_ptr;
struct queue; typedef struct queue* queue_ptr;
typedef void* payload_ptr;
struct queue { q_type_ptr tail; q_type_ptr head; uint length; };
struct q_type { q_type_ptr next; q_type_ptr prev; payload_ptr payload; };
