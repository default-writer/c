...
u32 pointer_size(struct pointer* ptr);
void pointer_push(struct pointer* ptr);
struct pointer* pointer_peek();
struct pointer* pointer_pop();
void pointer_push_void(void* ptr);
void* pointer_peek_void();
void* pointer_pop_void();
void pointer_init();
void pointer_destroy();
struct pointer* pointer_new(u32 size);
struct pointer* pointer_new_file(FILE* file);
void pointer_delete(struct pointer* ptr);
...
void* pointer_data(struct pointer* ptr) {
    if (ptr->type == TYPE_FILE) {
        ...
        u32 data_size = pointer_size(ptr) + 1;
        struct pointer* data_ptr = pointer_new(data_size);
...
struct pointer* pointer_match_last(struct pointer* src_ptr, struct pointer* match_ptr) {
    ...
    if (match != 0 && *match != 0) {
        ...
        if (last != 0 && *match == 0) {
            struct pointer* last_match_ptr = pointer_new(0);
...
const struct pointer_methods pointer_methods_definition = {
    ...
    .peek = pointer_peek,
    .pop = pointer_pop,
    .push_void = pointer_push_void,
    .peek_void = pointer_peek_void,
    .pop_void = pointer_pop_void,
    .data = pointer_data,
    .size = pointer_size,
