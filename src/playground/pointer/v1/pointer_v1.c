...
static void pointer_list_free(u64 ptr) {
    if (ptr != 0) {
        ...
        if (list_ptr->data != 0) {
            ...
            global_free(list_ptr->data, list_ptr->size);
#ifdef USE_MEMORY_CLEANUP
            list_ptr->data = 0;
            list_ptr->size = 0;
#endif
        }
        pointer_free_internal(list_ptr);
    }
}
...
const struct pointer_methods pointer_methods_definition = {
    ...
    .file_alloc = pointer_open_file,
    .file_read = pointer_read_file,
    .file_free = pointer_close_file,
